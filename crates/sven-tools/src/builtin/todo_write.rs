...
impl Tool for TodoWriteTool {
    ...
    fn description(&self) -> &str {
        "Structured task list for the session. Update silently — don't announce todo updates.\n\
         Replaces the entire list on each call. At most one in_progress at a time (enforced).\n\
         Mark completed IMMEDIATELY after finishing. Start the first item as in_progress and \
         begin work in the same turn.\n\
         Use for: complex multi-step tasks (3+ steps), non-trivial planning, multiple user tasks.\n\
         Skip for: trivial/single tasks, conversational requests, adding test-only tasks.\n\
