...
/// Maximum bytes read from a single git sub-command output.  Prevents
/// pathological cases (e.g. thousands of changed files in `git status`).
const GIT_OUTPUT_LIMIT: usize = 4 * 1024; // 4 KB

/// Per-command timeout for git sub-processes.  Avoids hanging sven on slow
/// or broken repositories (e.g. missing network drives, corrupt pack files).
...
/// Run a git command in `dir` with a hard timeout.  Returns the trimmed
/// stdout, or `None` if the command fails, times out, or produces no output.
...
/// Maximum bytes loaded from a project context file.  Files larger than this
/// are truncated to avoid flooding the context window with tokens.
/// (~16 KB ≈ 4 000 tokens — generous for project instructions.)
...
/// 2. `AGENTS.md`          — standard agent instructions (OpenAI Codex, etc.)
/// 3. `CLAUDE.md`          — Claude Code project file
///
/// Files larger than [`MAX_CONTEXT_FILE_BYTES`] are loaded up to the limit
/// and a truncation notice is appended.
///
/// Returns the trimmed file contents when a non-empty file is found.
        ...
        match std::fs::read(path) {
            ...
            Ok(bytes) => {
                let (content, truncated) = if bytes.len() > MAX_CONTEXT_FILE_BYTES {
                    ...
                    // Trim to the last valid UTF-8 char boundary
...
impl CiContext {
    ...
    /// Format the context as a system prompt section.  Returns `None` when
...
/// variables in workflow files.  These have the **lowest** precedence — they
/// are overridden by frontmatter `vars` and CLI `--var` flags.
///
/// Variable names follow the pattern `CI_<ORIGINAL_NAME>` where
/// `ORIGINAL_NAME` is the env var name in lower-case, e.g.:
/// - `{{CI_GITHUB_SHA}}`
/// - `{{CI_CIRCLE_BRANCH}}`
///
/// In addition the following common shortcuts are provided (without prefix):
/// - `{{branch}}` — the current branch from any provider
/// - `{{commit}}` — the current commit from any provider
/// - `{{repo}}` — the repository name/URL from any provider
/// - `{{pr}}` — the PR/MR number from any provider
pub fn ci_template_vars(ci: &CiContext) -> std::collections::HashMap<String, String> {
    let mut vars = std::collections::HashMap::new();

    // Provider-specific raw env vars — injected with CI_ prefix.
    let raw_vars: &[&str] = &[
    ...
    // Convenient shortcuts regardless of provider.
...
mod tests {
    ...
    fn load_project_context_file_missing_returns_none() {
        let tmp = std::env::temp_dir().join("sven_test_no_ctx");
    ...
    fn load_project_context_file_reads_agents_md() {
        let tmp = std::env::temp_dir().join("sven_test_agents_md");
        let _ = std::fs::create_dir_all(&tmp);
    ...
    #[test]
    fn load_project_context_file_truncates_large_files() {
        let tmp = std::env::temp_dir().join("sven_test_large_ctx");
        let _ = std::fs::create_dir_all(&tmp);
        let path = tmp.join("AGENTS.md");
        // Write a file larger than the cap
        let big_content = "x".repeat(MAX_CONTEXT_FILE_BYTES + 100);
        std::fs::write(&path, &big_content).unwrap();
        let result = load_project_context_file(&tmp);
        let _ = std::fs::remove_file(&path);
        let content = result.expect("should find the file");
        assert!(content.len() <= MAX_CONTEXT_FILE_BYTES + 100); // has truncation notice
        assert!(content.contains("truncated"));
    }

    #[test]
    fn load_project_context_file_prefers_sven_context_md() {
        let tmp = std::env::temp_dir().join("sven_test_pref_ctx");
        let _ = std::fs::create_dir_all(&tmp);
        let sven_dir = tmp.join(".sven");
        std::fs::create_dir_all(&sven_dir).unwrap();
        std::fs::write(sven_dir.join("context.md"), "Sven-specific instructions.").unwrap();
        std::fs::write(tmp.join("AGENTS.md"), "Generic agent instructions.").unwrap();
        let result = load_project_context_file(&tmp);
        let _ = std::fs::remove_dir_all(&tmp);
        let content = result.expect("should find .sven/context.md");
        assert!(content.contains("Sven-specific"));
    }
