...
mod tests {
    ...
    fn render_scroll_zero_maps_row_zero_to_first_output_line() {
        ...
        assert!(
            row0.contains('A'),
            "first output line comes from grid row 0"
        );
        assert!(
            row1.contains('B'),
            "second output line comes from grid row 1"
        );
    }
    ...
    fn render_merges_adjacent_cells_with_same_attr_into_one_span() {
        ...
        assert_eq!(
            span_count, 1,
            "four cells with the same attr_id must merge into one span"
        );
    }
